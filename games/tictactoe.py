from typing import Any, Dict, List, Optional
from games.interface import GameInterface

class TicTacToe(GameInterface):
    """
    Concrete implementation of GameInterface for Tic-Tac-Toe.
    """
    
    def __init__(self) -> None:
        # 1D array of 9 elements. None means empty cell.
        self.board: List[Optional[str]] = [None] * 9
        # Default starting player is X
        self.current_turn: str = 'X'

    def get_state(self) -> Dict[str, Any]:
        """Return the current game state as a JSON-serializable dictionary."""
        return {
            "board": self.board,
            "current_turn": self.current_turn,
            "status": self.is_game_over() or "ACTIVE"
        }

    def get_valid_moves(self) -> List[int]:
        """Return a list of valid moves (indices of empty cells) for the current state."""
        return [i for i, cell in enumerate(self.board) if cell is None]

    def apply_move(self, player: str, move: Any) -> bool:
        """
        Apply a move for the given player.
        Returns True if the move was valid and successfully applied, False otherwise.
        """
        if not isinstance(move, int):
            return False
            
        if move < 0 or move > 8:
            return False
            
        if self.board[move] is not None:
            return False
            
        if player != self.current_turn:
            return False
            
        self.board[move] = player
        self.current_turn = 'O' if player == 'X' else 'X'
        return True

    def is_game_over(self) -> Optional[str]:
        """
        Check if the game is over.
        Returns the winning player's identifier ('X' or 'O'), 'draw', or None if the game is still active.
        """
        winning_vectors = [
            (0, 1, 2), (3, 4, 5), (6, 7, 8), # Rows
            (0, 3, 6), (1, 4, 7), (2, 5, 8), # Cols
            (0, 4, 8), (2, 4, 6)             # Diags
        ]
        
        for a, b, c in winning_vectors:
            if self.board[a] is not None and self.board[a] == self.board[b] == self.board[c]:
                return self.board[a]
                
        if all(cell is not None for cell in self.board):
            return "draw"
            
        return None
