...
>
> **Local-Only Rule**: This skill NEVER runs `git push` (or any other command that mutates a remote — `push --tags`, `git push --force`, etc.). Every git operation below (`add`, `commit`, `tag`) is local only. All commits, releases, and tags created during this run stay on the local branch until the user explicitly pushes them separately.
