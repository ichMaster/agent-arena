...
- `/release-version 01.01.00 Server skeleton; health endpoints` -- bump with provided changelog items
...
Version notation `vXX.YY.ZZ`:
- `XX` = Phase number (e.g., Phase 1 -> 01)
- `YY` = Iteration within the phase (e.g., 01)
- `ZZ` = Post-release fix on that phase (e.g., 00)
Releases are cut per phase. **Never change the version without explicit user confirmation.**

## Instructions

### Step 0: Parse arguments

1. Extract the target version from the first argument (e.g., `01.01.00`)
2. Remaining arguments (separated by `;`) become changelog bullet points
3. Validate version format matches `XX.YY.ZZ`
...
1. **`VERSION`** (create if it doesn't exist): the bare version string, e.g. `01.01.00`
2. **`README.md`** (if present): update version reference if explicitly mentioned
3. **`server/main.py`** (if present): update the `version=""` argument inside the FastAPI instantiation.
4. **`RELEASE.txt`** (create if it doesn't exist): prepend a new version block at the top (after any header):

   ```
   Version v<version> (YYYY-MM-DD)
   ---------------------------
   - <changelog item 1>
   - <changelog item 2>
   ```
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt, server/main.py version string), never logic code.
