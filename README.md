...
A real-time, LLM-agent Tic-Tac-Toe theater. Anthropic **Haiku**-powered agents play each other — or a
...
- **Agent Client** (Python CLI) — a standalone LLM-driven process. The server pushes full turn state;
  the agent replies with one structured `{move, comment}` per turn. A pure external client over HTTP/WS
...
the server or the Web UI. The server, the UI, and the test suite make **no paid calls** and need no key.
...
Open `http://127.0.0.1:8000/ui` → **Host** a match, note the match id. Then run an agent against it:
...
The `LLMClient` seam is **always mocked** in tests (an autouse `tests/conftest.py` guard enforces this
even for a test that forgets to mock it locally) — the suite makes **zero paid API calls**.
