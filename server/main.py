"""The AgentArena FastAPI app — lifespan, health, and (this phase) the substrate for the lobby REST.

Built with a `lifespan` context (no deprecated startup hooks, §3) that runs `init_models` on startup.
The REST surface lives under `/api/v1/*`. The WebSocket hub and the `/ui` static mount arrive in later
phases. `create_app` builds the app bound to a given engine/session maker so tests never touch
`./arena.db`; `app = create_app()` (bottom) is the process-wide ASGI app for `uvicorn server.main:app`.
"""
...
APP_VERSION = "04.02.00"  # bumped by the release process on each phase release
...
async def get_repository(request: Request) -> AsyncIterator[Repository]:
    """FastAPI dependency: a `Repository` over a per-request `AsyncSession` (§3, §10)."""
    ...
    try:
        ...
        while True:
            try:
                ...
                # absent on a binary frame. Same "malformed, don't kill the socket" contract.
        ...
        # client-initiated drop can surface as an async cancellation rather than WebSocketDisconnect
        # (§10) -- shield the release so that same cancellation can't also interrupt its own await
        # mid-commit and leak the seat.
...
def create_app(
    *,
    ...
    async def ws_match(websocket: WebSocket, match_id: str) -> None:
        await _ws_connection(websocket, match_id, manager, resolved_maker)
    ...
    async def join_match(
        ...
        token = issue_token()
        # The observer flag is written at join, BEFORE any seat is assigned (§6.3) -- so an
        # observer can never later be handed a seat.
