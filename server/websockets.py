"""The WebSocket protocol layer (architecture.md §5.3, §6.2) — pure and DB-free.

The `ConnectionManager` (per-match sockets + owner map) and the wire builders for the five
server→client events + the client→server action envelope. Data comes in as arguments; the
endpoint/handlers gather it from the DB. This module pins the §6.2 wire seam.
"""
...
# --- Envelope + event builders (§6.2). Pure functions returning {"event": ..., "payload": {...}}. ---
...
def state_update_event(
    board: list[Any], current_turn: str | None, valid_moves: list[Any], last_move: dict[str, Any]
) -> dict[str, Any]:
    return event(
        "state_update",
        {
...
class ConnectionManager:
    """Per-match live sockets (§5.3) — the only in-memory server state; everything durable is in SQLite.

    Touched only from the single event loop (no `await` mid-mutation of the dicts), so no locks.
    """
    ...
    def disconnect(self, match_id: str, websocket: WebSocket) -> str | None:
        """Remove the socket + owner; return its participant_id so the caller can release the seat."""
    ...
    async def broadcast(self, match_id: str, message: dict[str, Any]) -> None:
        """Serialize once, send to each socket, prune any that fails mid-send (a dead client never
        blocks delivery to the rest, §5.3)."""
    ...
    async def close_room(self, match_id: str) -> None:
        """Close every socket in a finished match (§5.3)."""
        for websocket in list(self._connections.get(match_id, [])):
            try:
