...
A profile packages a distinct agent identity into a runnable config without touching core logic: the
persona (``system_prompt``), the vendor tier (``model_type`` — fed verbatim to ``create_llm_client``),
the sampling temperature, and the short-term memory length. Profiles are YAML files under ``profiles/``.
"""
