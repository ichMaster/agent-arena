"""Integration tests for the /ui static mount + no-store middleware (ARENA-OPUS-025). No LLM."""
...
def test_ui_asset_carries_no_store(client: TestClient) -> None:
    # index.html is a real served asset; explicitly requesting it must also be no-store.
...
def test_prefixed_non_ui_path_not_no_store(client: TestClient) -> None:
    """Code review #1: a path merely prefixed '/ui' (not the mount) must not get no-store."""
