"""create_llm_client + load_api_key tests (ARENA-OPUS-OPUS-018). SDK mocked; no network, no paid call."""
...
def test_haiku_dispatch_returns_anthropic_haiku_client() -> None:
    ...
    ctor.assert_called_once_with(api_key="sk-test-key")  # the real SDK class never ran
...
def test_load_api_key_aborts_when_missing_or_empty(env: dict[str, str]) -> None:
