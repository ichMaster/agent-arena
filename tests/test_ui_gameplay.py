"""Served-asset tests for the Player-role gameplay wiring (v03.02). No LLM, no paid call.

The no-build UI is validated by asserting app.js's entry points + interactivity gate at the served
level (no JS engine in pytest), consistent with the vanilla-JS design.
"""
...
def test_interactivity_gate_on_my_turn(app_js: str) -> None:
    ...
    # Non-playable cells carry the real disabled attribute (a11y §9), not just a class.
    assert "cell.disabled" in app_js


def test_read_events_call_renderers(app_js: str) -> None:
    # joined and state_update both drive the board/players render (no optimistic state).
