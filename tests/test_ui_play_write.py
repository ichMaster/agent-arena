"""Served-asset tests for the Player-role write path (ARENA-OPUS-OPUS-029). No LLM, no paid call."""
...
def test_click_handler_sends_submit_move(app_js: str) -> None:
    ...
    assert "if (!isGameActive) return" in app_js  # off-turn / finished clicks ignored


def test_game_over_freezes_and_marks_win(app_js: str) -> None:
    assert "function handleGameOver" in app_js
    idx = app_js.index("function handleGameOver")
