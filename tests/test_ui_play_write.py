"""Served-asset tests for the Player-role write path (ARENA-OPUS-029). No LLM, no paid call."""
...
def test_click_handler_sends_submit_move(app_js: str) -> None:
    ...
    # guarded: ignores non-playable / disabled cells and inactive game (no optimistic update)
...
def test_game_over_freezes_and_marks_win(app_js: str) -> None:
    ...
    assert "'win'" in body            # winning line highlighted
    assert "WIN_LINES" in body        # derived from the 8 lines
    # game_over routes to the handler
...
def test_new_match_resets_play_state(app_js: str) -> None:
    ...
    # joinAndConnect resets on every new connection (§7 regression guard)
